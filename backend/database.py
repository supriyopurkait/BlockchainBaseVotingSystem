...
def setup_database():
    ...
    # Creation of the voters database
...
def update_database_from_blockchain():  
    ...
    # Fetch total candidates from the blockchain
    candidate_length = contract.functions.totalCandidates().call()

    # Fetch candidates from the blockchain and insert into the database
...
def insert_data(name, area, phone_number, document_number, wallet_address, doc_image):
    wallet_address = wallet_address.strip().lower()
    try:
        with sqlite3.connect('backend/db/voter_data.db') as conn:
            cursor = conn.cursor()
            cursor.execute('''
                INSERT INTO voters (name, area, phone_number, document_number, wallet_address, doc_image)
                VALUES (?, ?, ?, ?, ?, ?)
            ''', (name, area, phone_number, document_number, wallet_address, doc_image))
            conn.commit()
...
if __name__ == '__main__':
    setup_database()
    # Fetch and update the database with candidates from the blockchain
